...
function QuantityControl({ 
  ...
  const handleIncrease = () => {
    if (quantity < maxStock) {
      ...
      Alert.alert('وصلت للحد الأقصى', `لا يمكن إضافة المزيد من "${itemName}".\n\nالمخزون الكامل: ${maxStock}\nالكمية الحالية: ${quantity}\n\nوصلت للحد الأقصى المتاح!`);
    }
  };
...
export default function CartScreen() {
  ...
  useEffect(() => {
    // السلة محلية فلا نحتاج لتحميل من الباك اند
  }, [token]);
  ...
  const handleUpdateQuantity = async (productId: string, newQuantity: number) => {
    ...
    try {
      ...
      Alert.alert('خطأ', error.message || 'فشل في تحديث الكمية');
    }
  };
