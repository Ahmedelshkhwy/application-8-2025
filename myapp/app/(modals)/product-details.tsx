...
export default function ProductDetails() {
  ...
  const handleAddToCart = async () => {
    ...
    // التحقق من المخزون الحالي مع ما في السلة
    ...
    // إذا كانت الكمية الجديدة ستتجاوز المخزون، أظهر رسالة خطأ
    if (newTotalQuantity > product.stock) {
      Alert.alert(
        'تجاوز المخزون', 
        `لا يمكن إضافة هذه الكمية!\n\nالمخزون الكامل: ${product.stock}\nالكمية في السلة: ${currentQuantityInCart}\nتريد إضافة: ${quantity}\n\nالمجموع سيصبح: ${newTotalQuantity}`
      );
      return;
    }
  ...
  const handleQuantityChange = (increment: boolean) => {
    // التحقق من الحد الأقصى مع ما في السلة
    const existingInCart = state.items.find(item => item.id === product._id);
    const currentQuantityInCart = existingInCart ? existingInCart.quantity : 0;
    const maxAvailable = product.stock - currentQuantityInCart;

    if (increment && quantity < maxAvailable) {
          ...
          >
            <Ionicons 
              ...
              color={(() => {
                ...
                const currentQuantityInCart = existingInCart ? existingInCart.quantity : 0;
                const maxAvailable = product.stock - currentQuantityInCart;
                return quantity >= maxAvailable ? "#ccc" : "#23B6C7";
              })()} 
            />
          </TouchableOpacity>
        </View>
