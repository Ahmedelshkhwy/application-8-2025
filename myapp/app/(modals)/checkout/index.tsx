...
export default function CheckoutScreen() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // محاولة إرسال للباك اند
      try {
        console.log('🌐 محاولة إرسال الطلب للباك اند...');
        ...
        const orderData = {
          ...
          shippingAddress: {
            ...
            city: userCity || 'غير محدد',
            postalCode: userLocation ? `${Math.floor(userLocation.lat)}${Math.floor(userLocation.lng)}` : '00000',
            state: userCity || 'غير محدد',
            zipCode: userLocation ? `${Math.floor(userLocation.lat)}${Math.floor(userLocation.lng)}` : '00000',
            phone: user?.phone || ''
          }
        };

        const result = await createOrder(orderData, token);
        console.log('✅ نجح إرسال الطلب للباك اند');
        
        // تحديث المخزون في الباك اند
        console.log('🔄 تحديث المخزون في الباك اند...');
        // TODO: إضافة دالة تحديث المخزون
        
        // إظهار رسالة النجاح
        Alert.alert(
          'تم الطلب بنجاح',
          `تم إرسال طلبك بنجاح!\nرقم الطلب: ${result.order?._id || localOrder._id}\nسيتم التواصل معك قريباً لتأكيد الطلب`,
          [
            {
              text: 'حسناً',
              onPress: () => {
      ...
      // إفراغ السلة في النهاية - بعد كل الأحداث
      console.log('🧹 إفراغ السلة - الخطوة الأخيرة...');
      clearCart();
      console.log('✅ تم إفراغ السلة نهائياً');
